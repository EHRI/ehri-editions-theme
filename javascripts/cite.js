...
jQuery( document ).ready(
	function ($) {
		...
		if (typeof CitationInfo === 'undefined') {
			console.warn( 'No DOI found for citation widget' );
			return;
		}
		...
		var $loading        = $( '.citation-loading' );
		var $formats        = $( '.citation-formats' );
		var $errorEl        = $( '.citation-error' );
		var $citationText   = $( '#citation-text' );
		var $copyButton     = $( '#copy-citation' );
		var $formatSelector = $( '#citation-format-selector' );
		var $copiedNotice   = $( '.citation-copied' );

		// Not jquery.
		var dialog              = document.querySelector( '.citation-dialog' );
		var showCitationDialog  = document.getElementById( 'show-citation-dialog' );
		var closeCitationDialog = document.getElementById( 'close-citation-dialog' );
		...
		function interceptXHRUrl(originalUrl, replacementUrl, once) {
			...
			XMLHttpRequest.prototype.open = function(method, url, async, user, password) {
				if ( ! intercepted && typeof url === 'string' && url.includes( originalUrl )) {
					...
					var newUrl = url.replace( originalUrl, replacementUrl );
		...
		try {
			...
			if ( CitationInfo.resolver ) {
				interceptXHRUrl( 'https://doi.org/', CitationInfo.resolver, true );
			}
			var cite = new Cite( CitationInfo.doi );
			...
			$formatSelector.on( 'change', function() { updateCitationText( cite ) } );
			// Copy citation.
			$copyButton.on(
				'click',
				function () {
